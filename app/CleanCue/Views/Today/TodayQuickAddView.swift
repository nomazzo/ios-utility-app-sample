...
struct TodayQuickAddView: View {
    ...
    private var templateSections: [TodayQuickAddSection] {
        let sections = (provider.places + [provider.homeMaintenancePlace]).map { presetPlace in
            TodayQuickAddSection(
                place: presetPlace,
        ...
        return sections.compactMap { section in
            ...
            return TodayQuickAddSection(place: section.place, tasks: filteredTasks)
        }
    }
    ...
    var body: some View {
        NavigationStack {
                    ...
                    Section(section.place.localizedDisplayName) {
                        ForEach(section.tasks) { task in
                            Button {
                                ...
                                TodayQuickAddTaskRow(
                                    task: task,
                                    destinationText: destinationText(for: section.place),
                    ...
                    Button {
                        ...
                        Label(
                            activePlaces.isEmpty
                                ? L10n.text("places.add", "場所を追加")
                                : L10n.text("common.manualAdd", "手入力で追加"),
                            systemImage: activePlaces.isEmpty ? "house.badge.plus" : "square.and.pencil"
                        )
                    }
                }
            }
            .navigationTitle(L10n.text("today.add.title", "掃除を追加"))
            .searchable(
                text: $searchText,
            ...
            .confirmationDialog(
                L10n.text("pro.limit.title", "無料版の上限に達しました。"),
                ...
                Text(L10n.text("pro.limit.addTaskMessage", "Proにすると、場所や掃除の数を気にせず使えます。"))
            }
        }
    }
    ...
    private func add(_ presetTask: PresetTask) {
        ...
        do {
            let destinationPlace = try place(for: presetTask)
            ...
            processingTaskIDs.remove(presetTask.id)
            message = L10n.text("template.place.duplicateShort", "この場所はすでにあります。")
            warningFeedback += 1
        } catch TodayQuickAddError.planLimitReached {
            processingTaskIDs.remove(presetTask.id)
            showingProLimit = true
            warningFeedback += 1
        } catch {
    ...
    private func place(for presetTask: PresetTask) throws -> Place {
        if let existingPlace = creationService.existingPresetPlace(
            placeID: presetTask.placeID,
        ...
        guard featureGate.placeLimit(
            ...
            throw TodayQuickAddError.planLimitReached
        }

        return try creationService.createPresetPlace(
            placeID: presetTask.placeID,
    ...
    private func isAlreadyAdded(_ presetTask: PresetTask) -> Bool {
        ...
        guard let place = creationService.existingPresetPlace(
            placeID: presetTask.placeID,
            existingPlaces: activePlaces
        ) else {
            return false
    ...
    private func destinationText(for presetPlace: PresetPlace) -> String {
        if creationService.existingPresetPlace(placeID: presetPlace.id, existingPlaces: activePlaces) != nil {
            return L10n.text("today.add.destination.existing.short", "既存")
        }
        return L10n.text("today.add.destination.new.short", "新規")
    }
...
private enum TodayQuickAddError: Error {
    case planLimitReached
}

private struct TodayQuickAddSection: Identifiable {
...
private struct TodayQuickAddTaskRow: View {
    let task: PresetTask
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                HStack(spacing: 10) {
                    InlineMetaLabel(text: destinationText, systemImage: "house")
                    InlineMetaLabel(
